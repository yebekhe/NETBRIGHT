...
num_fragment = (
    14  # total number of chunks that ClientHello devided into (chunks with random size)
)
fragment_sleep = 0.01  # sleep between each fragment to make GFW-cache full so it forget previous chunks. LOL.

log_every_N_sec = (
    10  # every 30 second , update log file with latest DNS-cache statistics
)
...
class DNS_over_Fragment:
    ...
    def query(self, server_name):
        ...
        if offline_ip != None:
            ...
            log_write(f"offline DNS -->, {server_name},{ offline_ip}\n")
            return offline_ip

        cache_ip = DNS_cache.get(server_name, None)
        if cache_ip != None:
            print("cached DNS -->", server_name, cache_ip)
            log_write(f"cached DNS -->, {server_name}, {cache_ip}\n")
            return cache_ip
        ...
        log_write(f"online DNS Query, server_name\n")
        try:
            ...
            if (
                ...
                log_write(f"online DNS --> Resolved {server_name} to {resolved_ip}\n")
                return resolved_ip
            else:
                print(f"Error: {ans.status_code} {ans.reason}")
                log_write(f"Error: {ans.status_code} {ans.reason}\n")
...
class ThreadedServer(object):
    ...
    def handle_client_request(self, client_socket):
        ...
        if data[:7] == b"CONNECT":
            ...
            log_write(f"redirect http to HTTPS, {q_url}\n")
            response_data = (
                "HTTP/1.1 302 Found\r\nLocation: "
                + q_url
            ...
            log_write(f"Unknown Method, {str(data[:10])}\n")
            response_data = (
        ...
        log_write(str(server_name) + "-->" + str(server_port) + "\n")
...
def main():
    ...
    log_write(f"Now listening at: 127.0.0.1: + {str(listen_PORT)}\n")
