            ...
            try:
                if first_flag:
                    ...
                    if data:
                        ...
                        print(
                            f"Using backend IP: {backend_ip}"
                        )  # Print the selected backend IP
                        log_write(f"Using backend IP: {backend_ip}\n")
...
def load_config(config_path):
    ...
    global listen_PORT, Cloudflare_IPs, Cloudflare_port, my_socket_timeout, first_time_sleep, accept_time_sleep, random_fragment
...
def main():
    ...
    log_write(f"Proxy server listening on 127.0.0.1:{listen_PORT}\n")
