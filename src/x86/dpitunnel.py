...
def send_data_in_fragment(data, sock):
    ...
    log_write(f'[SEND] Fragments size: {L_fragment} bytes')
    for i in range(0, len(data), L_fragment):
        fragment_data = data[i:i+L_fragment]
        logging.debug(f'[SEND:{i}] {len(fragment_data)} bytes')
        log_write(f'[SEND:{i}] {len(fragment_data)} bytes')
